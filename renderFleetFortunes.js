...
function renderFleet() {
  ...
  if (view === "status") {
    ...
    const shipRow = s => {
      ...
      if (dockedHere && def.role === "tanker") {
        ...
        if (localFuel > 0 && room > 0) {
          ...
          const loadVal = Math.max(0, Math.min(loadMax, tankerLoadQty[s.id] != null ? tankerLoadQty[s.id] : loadMax));
          loadBtn = `<input class="qty" id="loadqty-${s.id}" type="number" min="0" max="${loadMax}" value="${loadVal}" title="Fuel to load" />
        ...
        if ((s.fuel || 0) > 0) {
          ...
          const unloadVal = Math.max(0, Math.min(unloadMax, tankerUnloadQty[s.id] != null ? tankerUnloadQty[s.id] : unloadMax));
          unloadBtn = `<input class="qty" id="unloadqty-${s.id}" type="number" min="0" max="${unloadMax}" title="Fuel to unload" value="${unloadVal}" />
    ...
    if (idleTankers.length) {
      ...
      const already = ship.fuel || 0;   // dispatch carries exactly this — no auto top-up from local storage/hold
      ...
      tankerCard = `<div class="card"><h4>⛽ Dispatch a tanker run</h4>
        ...
        <div class="ship-stat" style="margin-top:8px"><span class="k">Load</span><span class="v">${already} fuel ⛽${already <= 0 ? " (dispatching empty)" : ""} · ${cycles} cycle(s)${destPlanet ? ` to ${destPlanet.name}` : ""}</span></div>
