...
const CHANGELOG = [
  ...
  { version: "2.124.0", notes: "New: 🧳 Passenger liners join the fleet — Transit Shuttle, Passenger Packet, Luxury Liner and Colony Ship, unarmed civilian hulls (tanky like a same-class warship, but flat half-speed and near-zero firepower) built at any Shipyard. Board them in your 🚚 Personal Convoy to sell paid tickets — at one of your own colonies the manifest is drawn from that colony's own population, anywhere else it's booked as passage — and debark to grow a destination colony's population, housing-capped, the same way a migrant wave does. A liner still carrying passengers is never destroyed by combat: it's crippled instead, its passengers automatically evacuated to safety, whether that hit lands in a convoy ambush or an 🛡️ Escort resettlement run. New: ⚓ Seize hull now works on a raided 🛳️ Luxury Liner too, carrying over a damaged fraction of its manifest into your fleet. New: a 🧳 Resettlement Job contract kind on the faction board, and a passenger-payload variant of Escort convoy contracts." },
  { version: "2.123.1", notes: "Fix: ⚓ Seize hull required a target to be BOTH pinned (engines dead) AND beaten to 35% hull — but ordinary Hull-targeted fire never touches engines, so a target could be ground down to nothing and still never become seizable unless you specifically switched to the Engines combat target. Now a target ground down to 15% hull is seizable outright regardless of engine status; pinning it first still unlocks seizing earlier, at the looser 35% threshold." },
  ...
  { version: "2.115.0", notes: "New: sacking a planet's ground garrison (⚔️ Raider tab's Raid this world) now hauls real cargo out of its warehouses too, not just credits — a mix weighted toward what the world is actually known for (its own deposits: ore off a mining world, biomass off an agri-world, ...) plus a spread of everyday goods that widens the more developed it is. It's loaded straight into your hold like any other plunder — same loot-share split if allies are cutting in, capped by whatever cargo room you actually have — on top of the existing credit haul, unchanged." },
  { version: "2.114.0", notes: "New: hunting down the pirates plaguing a world now actively eases its 🚨 planetary alert — the same shared meter raiding its shipping or assaulting it directly raises — on top of the usual bounty, influence and standing. A tougher catch (Dread Captain, Pirate Warlord) eases it more than a Rookie Corsair. It's a real active counter-play, not just faster passive decay: hammer a world with raids, then work the alert back down by policing it instead of only waiting it out." },
  { version: "2.113.0", notes: "New: the 🚨 planetary alert meter now answers to ordinary lane traffic too, not just full planet assaults — every Hauler, Merchant Freighter, Luxury Liner, Smuggler Runner or Faction Patrol you take down (🔭 Sweep) nudges the SAME shared meter its world's picket and garrison read, so hammering one world's shipping alone eventually makes its lane escorts (and, if you ever assault it, its defenses) run just as heavy as a besieged world's. New: each kill also disrupts the specific trade it was carrying — a new 🚨 disrupted tag on the Market tab flags exactly the commodities you've been cutting off (pick off Ore Haulers and Ore/Metals get scarcer and dearer right there; everything else stays put), fading back slowly once the killing stops." },
  { version: "2.112.0", notes: "New: raided worlds now remember. A 🚨 planetary alert meter climbs every time you assault a world (more on a full sack) and ratchets its orbital picket and 🏰 ground garrison harder for next time — up to 2 more patrol hulls, plus a real strength bump on both. A hot world also runs a wartime economy (pricier 🔫 Weapons/⛽ Fuel/⛓️ Metals, squeezed 📦 Consumer Goods/💠 Luxury) and its buildup sours the defending faction's standing with the rest of the sector a little further each cycle. It only stands down slowly at peace, far slower than it climbs — the ⚔️ Raider tab's Raid card shows the current reading (🕊️ at ease / 🚨 alert %), so hammering the same world repeatedly is a real, visible trade-off instead of an infinite piggy bank." },
  { version: "2.111.0", notes: "Changed: 🏴‍☠️ Raid this world is now a real two-phase campaign. Phase 1: the world's orbital patrols scramble as a whole picket (2-4 vessels by how lawful it is) fought with the same pooled-fire Target/Focus wave combat as an Escort ambush — clear every one of them first. Phase 2: only once its space defense is gone can you hit the 🏰 ground garrison (a dug-in fortress that never flees), and only breaking THAT sacks the surface and pays the ground plunder. All the while the defender's coalition — same faction, or one in 🤝 Alliance with it — may answer the distress call with reinforcement wings jumping in from other worlds (capped, and an allied wing flies its own flag, so killing it angers that faction too). Each patrol kill pays its own salvage and a modest Wanted/rep hit; the sack itself still carries the full act-of-war consequences." },
  { version: "2.110.0", notes: "Added: ⚔️ Raider now lets you go on the offense — pick a faction world (not your own base/colony) and hit 🏴‍☠️ Raid this world to break its orbital defenses and sack the surface, using the same weapon-targeting, ally-calling and no-quarter tools as any other raid. It's a real act of war: heavy Wanted, a deep rep hit with that faction (and core law), and defenses that scale with how lawful the world is — but the payoff scales with how developed it is too, and a Privateer Commission against that faction still pays out." },
  { version: "2.109.0", notes: "Added: 🏗️ Bases can now be raided just like colonies always could — build a 🛡️ Garrison (new base module) to defend one, same as a colony's. Every raid, on a base or a colony, is now claimed by an actual band from your 🏴‍☠️ Contacts list when one's a fit (a crew you're friendly with never raids you; a hostile one is the likelier culprit) instead of always faceless \"Pirates\"." },
  { version: "2.108.0", notes: "Changed: 💬 Talk's hire negotiation is now decided by the game itself, not the model — the odds of an ACCEPT/COUNTER/REJECT and the resulting price are rolled from the band's economics, personality and standing the instant you make an offer, and the model's only job is narrating that decision in character. This ends the whole run of negotiation-parsing bugs for good (nothing about the price is ever read back out of the model's words anymore), and means haggling now works identically well on any model — or even with Ollama unreachable altogether, falling back to a plain-spoken line instead of blocking the deal." },
  { version: "2.107.3", notes: "Fix: 💬 Talk's hire-negotiation parser was still too strict about how a model states its answer — a bare \"ACCEPT\"/\"Accept\" with no price, or (from smaller models) just a lone number with no ACCEPT/COUNTER/REJECT at all, both fell through as unrecognized and never struck a deal. Both now read correctly (a bare accept takes your own offer at face value; a bare number reads as their counter), and the negotiation prompt now includes a worked example, which smaller models in particular follow far more reliably than the instruction alone." },
...
Object.assign(window, {
  ...
  deployBattleGroup, recallBattleGroup, setBattleGroupPosture, setBattleGroupFormation, setPlayerFormation, setConvoyFormation,
